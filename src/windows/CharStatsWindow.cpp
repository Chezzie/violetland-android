...
CharStatsWindow::CharStatsWindow(Configuration* config,
	...
	addElement("perks", _("Perks:"), videoManager->RegularText,
			r+2*h, 3*h + yAdd, TextManager::LEFT, TextManager::MIDDLE);
	...
	addElements(labels, videoManager->RegularText,
			r+2*h, 4*h + yAdd, h, TextManager::LEFT, TextManager::MIDDLE);

	getElement("unstoppable")->setBlinking(true);
	getElement("poisonbullets")->setBlinking(true);
	getElement("bigcalibre")->setBlinking(true);
	getElement("nightvision")->setBlinking(true);
	getElement("looting")->setBlinking(true);
	getElement("widesight")->setBlinking(true);
	getElement("magneto")->setBlinking(true);

	/* addElement("explanation", _("Move finger over text to get explanation..."),
			videoManager->SmallText, config->Screen.Width / 2, h + yAdd,
...
void CharStatsWindow::refresh() {
	...
	int purchasedPerks = JNIBridge::singleton().getPurchasedPoints();

	oss.str("");
	if (0 == purchasedPerks) {
		oss << boost::format(_("Available perk points: %i"))
				% m_player->LevelPoints;
	} else {
		oss << boost::format(_("Available perk points: %i + %i"))
				% m_player->LevelPoints % purchasedPerks;
	}
	...
	addElements(stats, m_videoManager->RegularText, l, 4 * h + yAdd, h,
			TextManager::LEFT, TextManager::MIDDLE);

	getElement("strength")->setBlinking(true);
	getElement("agility")->setBlinking(true);
	getElement("vitality")->setBlinking(true);
	...
	for (unsigned i = 0; i < sizeof(perks) / sizeof(BoolString); ++i)
	{
		if (perks[i].f)
		{
			std::string id = perks[i].str;
			addElement(perks[i].str, "+", m_videoManager->RegularText,
					r, (4 + i) * h + yAdd, TextManager::CENTER, TextManager::MIDDLE);
			getElement(&id[1])->setBlinking(false);
		}
	}
}
...
void CharStatsWindow::increasePlayerParam(std::string elementName) {
#ifdef __ANDROID__
	int purchasedPerks = JNIBridge::singleton().getPurchasedPoints();
	if (m_player->LevelPoints > 0 || purchasedPerks > 0) {
#else
	if (m_player->LevelPoints > 0) {
		...
		if (m_player->LevelPoints > 0) {
			...
			JNIBridge::singleton().decreasePurchasedPoints();
		}
#else
		m_player->LevelPoints--;
#endif
