...
uint32_t g_timeInPause = 0;
uint32_t g_frameDeltaTime = 0;
...
HUD* hud;
StaticObject* splash;
const GLfloat splashBgColor = 0.4f;
...
void createTerrain() {
	...
	terrain = new Terrain(terrainSurface, baseTex, tiles, gameState->GameAreaSize);
...
void startGame(void*, std::string) {
	cam->setW(1600);
	...
	glClearColor(splashBgColor, splashBgColor, splashBgColor, 1);
...
void initInternationlization() {
	...
	/*setlocale(LC_ALL, ""); // temp: removing localization
...
void initSystem() {
	...
	std::string projectTitle = getProjectTitle();

#ifdef WIN32
    core::init_desc desc;
    desc.w = config->Screen.Width;
    desc.h = config->Screen.Height;
	desc.appName = projectTitle.c_str();
	desc.title = projectTitle.c_str();
    desc.companyName = "X-Teleport";
    core::init(&desc);
	win = core::getWindow();
#else
    LOGFMT("Creating window %dx%d.", config->Screen.Width, config->Screen.Height);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_EGL, 1);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 1);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 1);
    SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
    win = SDL_CreateWindow(projectTitle.c_str(),
		SDL_WINDOWPOS_CENTERED,
        SDL_WINDOWPOS_CENTERED,
        config->Screen.Width,
        config->Screen.Height,
        SDL_WINDOW_FULLSCREEN | SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    LOGFMT("SDL window: %p", win);
    SDL_GetWindowSurface(win); // Necessary call to make SDL shit working
#endif
...
#ifdef __ANDROID__
	if (player->Xp > 0)
		JNIBridge::singleton().reportScore(player->Xp, gameMode == GAMEMODE_WAVES);
#endif
...
#ifdef __ANDROID__
	JNIBridge::singleton().showInterstitial();
#endif
}
...
/*bool switchVolumeDown(void* sender, std::string elementName) {
...
void showAbout(void* sender, std::string);
void showShop(void* sender, std::string);
...
void createOptionsWindow() {
	...
	w->addElement("sectionsound", _("Sound"), videoManager->RegularText, l,
			6 * h, TextManager::LEFT, TextManager::MIDDLE);
	w->addElements(soundLabels, videoManager->RegularText, l + 3 * h, 8 * h,
			h * 2, TextManager::LEFT, TextManager::MIDDLE);
	...
	w->addElement("savereturn", _("Save and return"),
			videoManager->RegularText, l, 12 * h, TextManager::LEFT,
			TextManager::MIDDLE);
...
void refreshMainMenuWindow() {
	...
	if (gameState->Begun && !gameState->Lost) {
		yAdd = videoManager->RegularText->getHeight() * 1.6;
	}

	w->addElement("gamemode", strGameMode, videoManager->RegularText, r,
			videoManager->RegularText->getHeight() * 4.0f + yAdd, TextManager::LEFT,
			TextManager::MIDDLE);
}
...
void createMainMenuWindow() {
	...
	mainMenu->addHandler(Window::hdl_lclick, "about", showAbout);
	if (!JNIBridge::singleton().isKindle()) {
		mainMenu->addHandler(Window::hdl_lclick, "shop", showShop);
	}
...
void createHighscoresWindow() {
	...
	w->addElement("back", _("Back to main menu"), videoManager->RegularText, l,
			15 * h, TextManager::LEFT, TextManager::MIDDLE);
...
#ifdef __ANDROID__
	if (JNIBridge::singleton().showLeaderboard(gameMode == GAMEMODE_WAVES)) {
		return;
	}
#endif
	windows["mainmenu"]->CloseFlag = true;
...
void handleCommonControls() {
	...
	if (input->getPressInput(InputHandler::Menu)) {
		if (gameState->Charstats) {
			map<string, Window*>::iterator it = windows.find("charstats");
			if (it != windows.end()) {
				clearMap<std::string, Window*> (&windows); // close all other windows
			}
			gameState->Charstats = false;
		} else {
...
void handlePlayer(LifeForm* lf) {
	...
	int movementX = 0;
	int movementY = 0;
	if (input->fingerIdWasd != -1)
	{
		SDL_Rect r;
		hud->getButtonArea(HUD::AREA_WASD, &r);
		SDL_Point pt = input->fingerXY[input->fingerIdWasd];
		if (pt.x < r.x)
			pt.x = r.x;
		else if (pt.x > r.x + r.w)
			pt.x = r.x + r.w;
		if (pt.y < r.y)
			pt.y = r.y;
		else if (pt.y > r.y + r.h)
			pt.y = r.y + r.h;
		int xc = r.x + r.w/2;
		int yc = r.y + r.h/2;
		movementX = pt.x - xc;
...
#ifdef __ANDROID__
	lf->TargetX = lf->X + input->target.x;
	lf->TargetY = lf->Y + input->target.y;
#else
...
#ifdef __ANDROID__
	if (input->ptTeleport.x >= 0 && input->ptTeleport.y >= 0) {

		if (player->Teleports <= 0)
		{
			input->ptTeleport.x = -1;
			input->ptTeleport.y = -1;
		}
		else
		{
			ParticleSystem* partSys = new ParticleSystem();

			float X = input->ptTeleport.x / videoManager->WK - cam->getHalfW() + cam->X;
			float Y = input->ptTeleport.y / videoManager->HK - cam->getHalfH() + cam->Y;

			for (int i = 0; i < 50; i++) {
				Particle* spark = new Particle(X + (rand() % 200) - 100,
						Y + (rand() % 200) - 100, 128, 128,
						resources->ExplTex[0]);
...
void processGame() {
	...
	if (gameState->HighScore) {
		if (/*input->hasBeenValidated()*/true) {
			Player* player = (Player*) gameState->getLifeForm(playerId);
...
void drawGame() {
	...
	if (!gameState->Lost) {
		static const GLfloat light_pos[] = { 0.0, 0.0, 1.0, 1.0 };
		if (player->getLight()) {
			glEnable(GL_LIGHT0);
	...
	if (!gameState->Lost) {
		...
		if (/*player->getLaser()*/true) {
			...
			const GLfloat cols[] = {
				...
				1.0f, 0.0f, 0.0f, (cam->getW() - len) / cam->getW() * 0.75f
			};
		...
		if (/*player->getLight()*/tod < 0.35f) {
			...
			const GLfloat vtx[] = {
				wpnX, wpnY, 0.0f,
				player->X + len * cos(rad - width), player->Y + len * sin(rad - width), 0.0f,
				player->X + len * cos(rad + width), player->Y + len * sin(rad + width), 0.0f
			};
...
void runMainLoop() {
	while (gameState->Works) {
		...
		if (gameState->Begun) {
...
#ifdef __ANDROID__
				JNIBridge::singleton().showBanner(false);
#endif
				continue;
			}
			...
			if (!gameState->Paused && !gameState->Charstats) {
				processGame();
				processTerrain();
#ifdef __ANDROID__
				if (!gameState->Lost)
...
#ifdef __ANDROID__
				JNIBridge::singleton().showBanner(true);
#endif
			}
...
void unloadResources() {
	...
	if (weaponManager) delete weaponManager;
	if (monsterFactory) delete monsterFactory;

	playerId.clear();
	clearMap<std::string, Window*> (&windows);

	if (config) delete config;
	if (tempConfig) delete tempConfig;
...
{
	LOG("Showing shop");
	JNIBridge::singleton().purchasePoints();
}

#ifdef __ANDROID__
extern "C" JNIEXPORT jboolean JNICALL Java_com_xteleport_violetland_MainActivity_isInPoll(JNIEnv*, jobject) {
	return (g_isInPoll || g_isExit) ? JNI_TRUE : JNI_FALSE;
}

extern "C" JNIEXPORT jstring JNICALL Java_com_xteleport_violetland_MainActivity_getKey(JNIEnv* pEnv, jobject) {
	const char* k = "ENTER YOUR API KEY HERE";
	return pEnv->NewStringUTF(k);
}
#endif
