...
SDL_Surface *ImageUtility::loadImage(boost::filesystem::path fileName, float prescale) {
	...
	SDL_Surface *optimizedImage = image;

	if (prescale != 1.0)
		return ImageUtility::scaleSurface(optimizedImage, optimizedImage->w * prescale, optimizedImage->h * prescale);
	else
...
SDL_Surface *ImageUtility::createImage(TTF_Font *font, char fgR, char fgG,
	...
	if (quality == solid)
		...
		resulting_text = TTF_RenderUTF8_Shaded(font, text.c_str(), tmpfontcolor, tmpfontbgcolor);
	else if (quality == blended)
		resulting_text = TTF_RenderUTF8_Blended(font, text.c_str(), tmpfontcolor);

	// SDL_Surface *optimizedImage = SDL_DisplayFormatAlpha(resulting_text);
	// SDL_Surface* screenSurface = SDL_GetWindowSurface(SDL_GL_GetCurrentWindow());
	// SDL_Surface* optimizedImage = SDL_ConvertSurface(resulting_text, screenSurface->format, 0);
